import { useCounterStore } from '@/features/counter/stores/counter.store'
...
type MenuViewVm = ComponentPublicInstance & {
  items: MenuItem[]
}

const { mockSetAppLocale, mockRouter, mockI18n } = vi.hoisted(() => {
  const locale = { value: 'en' }

  return {
    mockSetAppLocale: vi.fn(),
    mockRouter: {
      push: vi.fn(),
    },
    mockI18n: {
...
// Mock SubComp
vi.mock('@/shared/components/menu/sub/SubComp.vue', () => ({
  default: {
    name: 'SubComp',
...
describe('MenuView', () => {
  ...
  it('contains all expected menu routes', () => {
    ...
    const vm = wrapper.vm as unknown as MenuViewVm
    ...
    for (const route of expectedRoutes) {
      expect(menuItems.some((item) => item.route === route)).toBe(true)
    }
  })
  ...
  it('has badge only on todos menu item', () => {
    ...
    const vm = wrapper.vm as unknown as MenuViewVm
    const todosItem = vm.items.find((item) => item.route === '/todos')
    const homeItem = vm.items.find((item) => item.route === '/')
