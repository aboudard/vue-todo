...
import ArraysInput from '@/features/todos/components/ArraysInput.vue'
import BaseInput from '@/features/todos/components/BaseInput.vue'
import type { Todo } from '@/features/todos/models/todo'
  ...
  <div>
    ...
    <div class="p-3 flex flex-col gap-3 justify-center">
      ...
      <div class="flex-row flex justify-center gap-4">
        <Form ref="todoForm" v-slot="$form" :initialValues :resolver @submit="onFormSubmit"
          class="basis-1/3 flex flex-col gap-4 w-full sm:w-56">
          <div class="flex flex-col gap-1">
            <InputText v-model="initialValues.title" name="title" type="text" placeholder="title" fluid />
            <Message v-if="$form.title?.invalid" severity="error" size="small" variant="simple">{{
              $form.title.error?.message
            }}</Message>
          </div>
          <div class="flex flex-col gap-1">
            <DatePicker dataType="string" updateModelType="date" dateFormat="dd/mm/yy" v-model="initialValues.dueDate"
              name="dueDate" placeholder="Due Date" fluid />
            <Message v-if="$form.dueDate?.invalid" severity="error" size="small" variant="simple">{{
              $form.dueDate.error?.message
            }}</Message>
          </div>
          <div class="flex flex-col gap-1">
            <ToggleSwitch @change="(e) => logChange((e.target as HTMLInputElement)?.checked)"
              v-model="initialValues.awesome" name="awesome" />
          </div>
          <div class="flex flex-col gap-1">
            <ToggleSwitch v-model="initialValues.trulyAwesome" name="trulyAwesome" />
          </div>
          <div class="flex flex-col gap-1">
            <BaseInput v-model:completed="initialValues.completed" v-model:hours="initialValues.hours" />
          ...
          <div class="flex flex-col gap-4">
            <ArraysInput :items="categories" v-model:selectedItem="initialValues.category"
          ...
          <div>
            <FormField v-slot="$field" name="username" class="flex flex-col gap-1">
              <input type="text" v-model="initialValues.username" placeholder="Username"
                :class="[{ error: $field?.invalid }]" v-bind="$field.props" />
