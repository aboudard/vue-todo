<template>
  ...
  <div v-for="(category, index) of items" :key="category.key" class="flex items-center gap-2">
    <Checkbox :inputId="`category-${index}`" @value-change="writeValue('selectedItems', $event)" name="categories"
      :value="category" v-model="selectedItems" />
    <span>{{ category.name }}</span>
    <RadioButton :inputId="`selectedItem-${index}-radio`" @value-change="writeValue('selectedItem', $event)"
      name="category" :value="category.key" v-model="selectedItem" />
  </div>
  <div>
...
import type { Option } from '@/features/todos/models/option';
...
function writeValue(key: 'selectedItem', value: string): void
function writeValue(key: 'selectedItems', value: Option[]): void
function writeValue(key: 'selectedItem' | 'selectedItems', value: string | Option[]) {
  if (key === 'selectedItem') {
    selectedItem.value = value as string
    emit('update:selectedItem', value)
  } else if (key === 'selectedItems') {
    selectedItems.value = value as Option[]
